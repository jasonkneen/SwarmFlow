...
with tabs[0]:
    ...
    workflow_tabs = st.tabs(["Workflow", "Functions", "Agents", "Steps"])
    ...
    with workflow_tabs[0]:
        ...
        workflow['llm_provider'] = st.text_input("LLM Provider", value=workflow.get('llm_provider', ''), key='llm_provider')
    ...
    with workflow_tabs[2]:
        ...
        for idx, agent in enumerate(agents):
            ...
            agent_expander = st.expander(f"{agent_name}: {agent_desc}", expanded=False)
            with agent_expander:
                # Delete / Refresh, using callback function
                col_refresh_delete = st.columns([0.5, 0.5])
                with col_refresh_delete[0]:
    ...
    with workflow_tabs[3]:
        ...
        for idx, step in enumerate(steps):
            ...
            step_expander = st.expander(f"Order-{step_order} ({step_agent}), {step_name} (output {step_output.get('type', 'string')}): {step_desc}", expanded=False)
            with step_expander:
                # Delete / Refresh, using callback function
                col_refresh_delete = st.columns([0.5, 0.5])
                with col_refresh_delete[0]:
                ...
                output['name'] = st.text_input("Output Name", value=output.get('name', ''), key=f'step_output_name_{idx}')
                ...
                for i, type_name in enumerate(type_list):
                    if type_name == output.get('type', 'string'):
                        type_index = i
                        break
                output['type'] = st.selectbox("Output Type", options=type_list, index=type_index, key=f'step_output_type_{idx}')
                output['key'] = st.text_input("Output Key (for json type)", value=output.get('key', ''), key=f'step_output_key_{idx}')
                output['post_processing'] = st.text_input("Post Processing", value=output.get('post_processing', ''), key=f'step_post_processing_{idx}')
                output['append_to'] = st.text_area("Each variable takes one line", value='\n'.join(output.get('append_to', [])), key=f'step_append_to_{idx}')
                step['output'] = output
