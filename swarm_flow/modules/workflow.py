...
from modules.swarm.types import *
from modules.config import llm_settings
...
class StepTask:
    ...
    async def run(self):
        ...
        output_var = agent_params["output"]["name"]
        output_type = agent_params["output"].get("type", "string")
        ...
        if "for_each" in agent_params:
            ...
            async def process_item(item):
                ...
                stop_character = agent_params.get("stop_character")
                if stop_character:
            ...
            if output_type == "list":
                output = self.extract_list(output)
                ...
                key = agent_params["output"].get("key", "")
                if len(key) > 0:
        ...
        # Process 'format'
        output_format = agent_params["output"].get("format")
        if output_format:
        ...
        # Process 'append_to'
        append_to = agent_params["output"].get("append_to")
        if append_to:
...
class WorkflowExecutor:
    def __init__(self, yaml_content, max_concurrency=5, messages=None, context_variables=None,
        ...
        self.llm_settings = llm_settings[provider]
