...
*注：工作流配置文件中`llm_settings`的`provider`字段，对应`config.py`文件中`llm_settings`的子项名称（例如`openai`），二者必须匹配。*
...
与此示例对应的工作流配置文件为`data/cn/base.yaml`。
...
与此示例对应的工作流配置文件为`data/cn/guide_to_reading.yaml`。 
...
示例为正反双方辩论模式，对应的工作流配置文件为`data/cn/debate_competition.yaml`。

### 3.3. 图形界面

运行`./run.sh`会在后台启动`streamlit`服务，可通过浏览器访问`http://localhost:8501`打开 UI 页面，便于编辑和调试工作流。

基本操作（以“文章导读”工作流为例）：
1. 点击“Browse files”按钮，从`data/cn`目录中选择`guide_to_reading.yaml`，或者通过拖拽加载该文件；
2. 查看或修改各项配置；
...
`data`目录中内置了一些工作流文件，此处以`base.yaml`和`debate_competition.yaml`为例。
...
# LLM 设置（必填字段）
llm_settings:
  # provider 必须与 config.py 中 llm_settings 的子项名称匹配。
  provider: "openai"
...
agents:
  - name: "AI助手"
    ...
    description: "AI助手，负责分发任务，并向用户返回结果"
    ...
    output:
      # 在其他 agent 或 step 中可以通过 {{ assistant_output }} 引用此变量
...
agents:
  ...
  - name: "主持人"
    ...
    # stop_character 为停止符，最终的输出内容将忽略 stop_character 后面的内容
    stop_character: "\n\n"
    ...
    output:
      name: speech
      type: json
      # format 为可选字段，用于格式化最终的输出结果，同时会将 output type 强制转换为 string
...
`examples/cn/debate_competition.py`中演示了外部代码如何引用`append_to`字段中的`conversation`变量。
...
在工作流中的应用方式可参考`data/cn/guide_to_reading.yaml`。

## 5. 后续计划

- [ ] 引用外部知识库：工作流中增加对 RAG（Retrieval Augmented Generation） 的支持。
